...
fn tx_offset() {
    ...
    TransactionFactory::from_seed(1295)
        ...
        .for_each(|(mut tx, _)| {
            ...
            if let Ok(slots) = tx.storage_slots() {
            ...
            tx.inputs()
                ...
                .for_each(|(idx, i)| {
                    ...
                    let i_p = Input::from_bytes(&bytes[input_ofs..])
                    ...
                    if let Some(utxo_id) = i.utxo_id() {
                        ...
                        let ofs =
                            input_ofs + i.repr().utxo_id_offset().expect("input have utxo_id");
                        let utxo_id_p = UtxoId::from_bytes(&bytes[ofs..])
                            .expect("failed to deserialize utxo id");
                    ...
                    if let Some(asset_id) = i.asset_id() {
                        ...
                        if let Some(offset) = i.repr().asset_id_offset() {
                            ...
                            let asset_id_p = unsafe {
                                AssetId::as_ref_unchecked(&bytes[ofs..ofs + AssetId::LEN])
                            };
                    ...
                    if let Some(contract_id) = i.contract_id() {
                        ...
                        let contract_id_p = unsafe {
                            ContractId::as_ref_unchecked(&bytes[ofs..ofs + ContractId::LEN])
                        };
                    ...
                    if let Some(message_id) = i.message_id() {
                        ...
                        let message_id_p = unsafe {
                            MessageId::as_ref_unchecked(&bytes[ofs..ofs + MessageId::LEN])
                        };
...
fn iow_offset() {
    ...
    TransactionFactory::from_seed(3493)
        ...
        .for_each(|(mut tx, _)| {
            ...
            tx.inputs().unwrap().iter().enumerate().for_each(|(x, i)| {
            ...
            if let Ok(offset) = tx.receipts_root_offset() {
                let receipts_root = rng.gen();

                tx.set_receipts_root(receipts_root).unwrap();
